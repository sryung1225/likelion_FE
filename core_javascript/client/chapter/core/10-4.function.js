...
let answer = pow(2, 53);
console.log(answer); // ? 9007199254740992
...
// 참고: https://bit.ly/memoiz / https://ko.reactjs.org/docs/react-api.html#reactmemo
// - 동일 계산 반복 시, 이전 계산 값을 메모리에 저장하여 실행 속도를 높이는 방법

// const cache = {}; // 로 선언하는건 전역이기 때문에 괜히 위험할 수 있음
const memoFibo = (n) => {
  if(n<=0) return 0;
  if(n<=2) return 2;
  if(memoFibo.cache[n]){
...
memoFibo.cache = {}; // const cache 대신에 사용한 선언 방식. memoFibo라고 있는 것을 사용
// console.log(memoFibo(100));
// console.log(memoFibo(1000));
// 덕분에 여러번 계산하지 않고 딱 한 번만 계산하기 때문에
